    ...
    <table style="width:100%;border-collapse:collapse;margin-top:14px;font-size:13px;">
      <tr>
        <td style="padding:8px;border:1px solid #e2e8f0;background:#f8fafc;text-align:center;width:25%;">
          ...
          <div style="font-weight:700;margin-top:2px;">${h.holding.shares.toLocaleString()} @ ${money(h.holding.avgPrice)}</div>
        </td>
        <td style="padding:8px;border:1px solid #e2e8f0;background:#f8fafc;text-align:center;width:25%;">
...
export function renderReportHtml(report: StoxReport): string {
...
<body style="margin:0;padding:0;background:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#0f172a;">
  <div style="max-width:820px;margin:0 auto;padding:24px 16px 48px;">
    ...
    <div style="background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%);color:#f8fafc;border-radius:16px;padding:28px 24px;margin-bottom:20px;">
      <div style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#94a3b8;font-weight:600;">AetherForge Stox · Full Engine · Self-hosted</div>
    ...
    <div style="text-align:center;margin-top:18px;font-size:11px;color:#94a3b8;">
      © Forge Intelligence Ltd · AetherForge AI · Self-hosted Stox package (full engine)
    </div>
  </div>
</body>
</html>`;
...
export function writeReport(report: StoxReport, outputDir = "output"): string {
  mkdirSync(outputDir, { recursive: true });
  const stamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
  const fileName = `stox-report-${stamp}.html`;
